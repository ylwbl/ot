...
const appendIcon = (arr, isChild: boolean = false) => {
  if (Array.isArray(arr)) {
    arr.forEach((item) => {
      if (item.children) {
        item['icon'] = ({ expanded }) =>
          expanded ? <ParentOpenTagListIcon /> : <ParentCloseTagListIcon />;
        appendIcon(item.children, true);
      } else {
        item['icon'] = ({ selected }) =>
          selected ? <BillWhite /> : <ChildTagListIcon />;
      }
    });
  }
};
...
export default class SearchTree extends React.Component<Props, State> {
  ...
  static getDerivedStateFromProps(nextProps: Props, prevState: State) {
    if (nextProps.dataIndex !== prevState.dataIndex) {
      const { allTreeData, allDataList, expandedKeys } = getInitData(
        nextProps.treeData || []
      );
      ...
      data.map((item) => {
        ...
        const title =
          index > -1 ? (
            <span>
              {beforeStr}
              <span className='searchTree-tree-searchValue'>{searchValue}</span>
              {afterStr}
            </span>
          ) : (
