...
import { ELImportExcel } from '@/components/el';
  ...
  <ELImport_3
          downTemplateConfig={{
            downloadType: 'templateCode',
            fileName: '销售报价单导出模板',
            templateCode: 'yst_sale_quotation'
          }}
          requestPath=''
...
interface Props {
  downTemplateConfig: {
    downloadType: 'templateCode' | 'request' | 'path';
    url?: string;
...
class ELImportExcel extends React.Component<Props, State> {
  ...
  render() {
    ...
    const { downloadType, requestMethod, url, fileName, templateCode } = this.props.downTemplateConfig;
    return (
      <Modal
        title={modalTitle}
        visible={visible}
      ...
      >
        <FileBox
          {...omit(['downTemplateConfig'], this.props)}
